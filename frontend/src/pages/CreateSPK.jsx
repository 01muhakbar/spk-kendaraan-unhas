...
const CreateSPK = () => {
  ...
  useEffect(() => {
    const fetchMasterData = async () => {
      try {
        const [resK, resV, resS, resNext] = await Promise.all([
          axios.get(`${API}/vehicles`),
          axios.get(`${API}/vendors`),
          axios.get(`${API}/signatories`),
          axios.get(`${API}/spk/next-number`)
        ]);
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="bg-gradient-to-r from-blue-700 to-blue-500 text-white p-6 rounded-xl mb-6 shadow-lg">
        <h2 className="text-2xl font-bold">Buat SPK Perbaikan Kendaraan</h2>
        <p className="text-blue-100 text-sm mt-1">Isi satu form — sistem akan mengisi otomatis ke 5 lembar surat</p>
      </div>
